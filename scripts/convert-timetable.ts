...
function parseRows(rows: RawRow[], nameToId: Map<string, number>): TimetableAct[] {
  // 첫 줄은 헤더, 나머지만 파싱
  return rows
    .slice(1)
...
async function convertTimetable(xlsxPath: string): Promise<Timetable> {
  const nameToId = buildNameToIdMap();

  const days: TimetableDay[] = [];

  for (const sheetName of Object.keys(DAY_META)) {
    const meta = DAY_META[sheetName];
    if (!meta) continue;

    const rows = await readSheet(xlsxPath, sheetName);
    const acts = parseRows(rows, nameToId);
...
async function writeOutput(data: Timetable, outPath: string): Promise<void> {
  ...
  const formatted = await format(header + body, { parser: 'typescript', singleQuote: true });
  fs.writeFileSync(outPath, formatted, 'utf-8');
}

// Entry point
if (require.main === module) {
  const xlsxPath = path.resolve(__dirname, '../docs/2026캠프 운영/timetable_v7.xlsx');
  ...
  convertTimetable(xlsxPath)
    .then(async (data) => {
      await writeOutput(data, outPath);

      const totalTeams = data.days.reduce((n, d) => n + d.teamCount, 0);
      process.stdout.write(`Converted: ${data.days.length} days, ${totalTeams} teams\n`);
    })
    .catch((error: unknown) => {
