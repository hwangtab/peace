...
## 3라운드 (2026-07-05) — Serif 슬라이스 분할

| 배치 | 내용 | 커밋 |
| ---- | ---- | ---- |
| F8 (Opus) | NotoSerifKR-Bold(508KB, 최대 단일 폰트)를 core(333KB, 공개 실사용 음절+자모+구두점)/rest(224KB, 잔여 상용 음절) 2슬라이스로 분할 — unicode-range가 자동 선택. 공개 3페이지 실측에서 core만 로드, core∪rest==기존 커버리지 검증으로 admin 동적 텍스트 무손실 | abb0bbdc |
| F9 | CrUX API 키 생성은 권한 정책 차단 — 사용자 결정 대기(직접 발급 또는 명시 허가) | — |

### 3라운드 결과 (홈 3회 중앙값)

| 지표 | 기준선 | 3R 후 | 누적 |
|---|---|---|---|
| 점수 | 59 | **64** (웜 캐시 관측 최고 80) | +5 (중앙값 기준) |
| LCP | 10.8s | **8.4s** | -2.4s |
| FCP | 4.9s | 4.8s | 1R 직후 1.7~1.8s 관측 — 캐시 민감 |
| 폰트 실측 | 1,490KB | **1,021KB** | **-469KB (-31%)** |

### 폰트 여정 요약 (ko 홈 즉시 로드)
1,490KB → (F1 Partial 재서브셋) → (F5 웨이트 4→2) 1,195KB → (F8 Serif 분할) **1,021KB**.
남은 바닥: KR Regular 234 + KR Bold 239 + Serif core 333 + Partial 103 + 라틴 ~110.
다음 단계가 있다면 본문 KR의 구글폰트식 다중 슬라이스(수십 조각)지만 파이프라인 복잡도
대비 랩 이득이 체감 구간을 지나 우선순위 낮음.
