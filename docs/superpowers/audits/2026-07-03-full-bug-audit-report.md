...
## 런타임 감사 회차 (2026-07-04)

배포 사이트(peaceandmusic.net)를 실 브라우저(puppeteer)로 크롤 — 210페이지(ko 전수 + en·ar·ja 대표 20 + 나머지 9로케일 스모크), 내부 링크 2,579개, 외부 출처 22개.
...
| hydration 실패 | 0건 |
| 미포착 JS 예외 | 0건 |
| 깨진 이미지(naturalWidth=0) | 0건 |
| 내부 링크 4xx/5xx | 0 / 2,579 |
| 문서 상태 이상·타임아웃·빈 title | 0건 |
| ar 로케일 RTL(dir="rtl") | 정상 |
| 커스텀 404 페이지 | 정상 ("페이지를 찾을 수 없습니다") |
| 외부 링크 | HEAD 404 7건은 전부 오탐(언론사 CMS의 HEAD 차단) — GET 재검증 시 200 |

### 유일한 관측: Supabase 무료티어 egress 402 (알려진 운영 장애의 사용자 체감)

- 210페이지 전부에서 콘솔 에러 1건: `GET /rest/v1/boards?...` 402. 내비 게시판 목록 조회인데, `useCommunityBoards`가 모듈 캐시·inflight 중복 제거·정적 폴백을 갖춰 **메뉴는 폴백으로 정상 표시**되고 세션당 1회 요청이라 egress 부담도 미미.
- 실질 영향: 402가 풀릴 때까지 **게시판 글 목록·상세, 로그인·회원 기능이 프로덕션에서 장애 상태**. 코드 결함이 아니라 무료티어 카운터 문제 — 월 주기 리셋 대기 또는 Pro 업그레이드로만 해소.

### 판정

정적 감사에서 수정한 10개 근본원인 외에 런타임에서 새로 드러난 코드 결함은 없음. 감사 스코프(정적+인프라+빌드+런타임) 전체 완료.
