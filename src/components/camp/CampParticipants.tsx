      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {participants.map((participant, index) => {
          ...
          return (
            ...
            >
              <span
                className={`inline-block w-2 h-2 rounded-full transition-colors duration-200 mt-1.5 ${clickable ? 'bg-jeju-ocean group-hover:bg-ocean-mist' : 'bg-jeju-ocean'}`}
              />
              <h3
                className={`typo-h3 !text-lg transition-colors duration-200 ${clickable ? 'text-jeju-ocean group-hover:text-ocean-mist underline underline-offset-4 decoration-jeju-ocean/30' : 'text-coastal-gray'}`}
              >
                {name}
              </h3>
            </motion.div>
