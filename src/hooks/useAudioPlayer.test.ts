...
describe('useAudioPlayer', () => {
  ...
  it('초기 상태: progress=0, duration=0, error=null', () => {
    const { result } = renderHook(() =>
      useAudioPlayer({ audioUrl: '', isPlaying: false }),
    );
  ...
  it('isPlaying=true → play 호출', async () => {
    renderHook(() =>
      useAudioPlayer({ audioUrl: 'http://example.com/track.mp3', isPlaying: true }),
    );
    await act(async () => {
      await Promise.resolve();
      await Promise.resolve();
  ...
  it('isPlaying=false → pause 호출', async () => {
    const { rerender } = renderHook(
      ({ isPlaying }) =>
        useAudioPlayer({ audioUrl: 'http://example.com/track.mp3', isPlaying }),
